...
fn get_file_path(args: &ArgMatches<'_>) -> Result<PathBuf, IoError> {
    ...
    let name = args.value_of("file")
        .unwrap(); //SAFETY: This is required in get_args(), therefore we know it's there and valid.
    let path = dir.join(name);

    if !path.exists() {
        return Err(IoError::new(ErrorKind::NotFound, "provided path does not exist"));
    }
...
fn create_share(args: &ArgMatches<'_>, config: &Config) -> Result<Share, IoError> {
    ...
    let ext = match input_file.extension() {
        Some(f) => f,
        None => return Err(IoError::new(ErrorKind::Other, "file does not have extension")),
    };

    let name = match input_file.file_stem() {
        Some(n) => n,
        None => return Err(IoError::new(ErrorKind::Other, "unable to extract name of file")),
    };
    ...
    hard_link(&input_file,format!("{}/hard_links/{}", config.file_store_location(), id))?;
    ...
    Ok(Share {
        ...
        ///XXX Remove these options, we no longer want to use them.
        website: true,
        wget: true,
    })
}
...
fn generate_link_url(share: &Share, config: &Config) -> String {
    ...
    format!("{}/{}/{}", config.server_address(), public_id, share.id())
}
